...
function porto_load_info_box_shortcode() {
    ...
    for ( $i = 1; $i <=3; $i++ ) {
        if ( isset( $porto_settings['fonts_custom'.$i] ) && !empty( $porto_settings['fonts_custom'.$i]['font-family'] ) ) {
            $custom_fonts[$porto_settings['fonts_custom'.$i]['font-family']] = $porto_settings['fonts_custom'.$i]['font-family'];
        }
    }
